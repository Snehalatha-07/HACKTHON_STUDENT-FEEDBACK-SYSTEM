...
// Animated horizontal SVG bar chart with palette + legend support.
// Props:
// - data: array of { id, name, count, color? }
// - labelKey/valueKey: keys for label and numeric value
// - heightPer: row height
// - labelWidth: left label column width
// - onBarClick: callback(row)
// - palette: optional array of colors to use when item.color is missing
const BarChart = ({ data = [], labelKey = 'name', valueKey = 'count', heightPer = 34, labelWidth = 180, onBarClick, palette = null }) => {
  const [animatedWidths, setAnimatedWidths] = useState([]);

  const max = data.length ? Math.max(...data.map(d => d[valueKey] || 0)) : 1;
  const viewWidth = 900;
  const chartWidth = viewWidth - labelWidth - 60;
  const height = Math.max(40, data.length * heightPer + 20);

  // staggered animation of bar widths
  useEffect(() => {
  ...
  const showTooltip = (ev, row) => {
    ...
    setTooltip({ visible: true, x: p.x, y: p.y, label: row[labelKey], value: row[valueKey], percent });
  };
  ...
  const moveTooltip = (ev) => {
    ...
    rafRef.current = requestAnimationFrame(() => {
      rafRef.current = null;
      const p = clampPos(pendingPos.current.x, pendingPos.current.y);
  ...
  const hideTooltip = () => {
    ...
    setTooltip({ visible: false, x: 0, y: 0, label: '', value: 0, percent: 0 });
  };

  // Build legend items when palette or per-item color present
  const usePalette = Array.isArray(palette) && palette.length;
  const legendItems = data.map((d, i) => ({ label: d[labelKey], color: d.color || (usePalette ? palette[i % palette.length] : null) })).filter(it => it.color);
    ...
    <div ref={wrapRef} className="bar-chart-wrap" style={{ width: '100%', overflow: 'auto', position: 'relative' }}>
      <svg className="bar-chart" viewBox={`0 0 ${viewWidth} ${height}`} width="100%" height={height} role="img" aria-label="Bar chart">
              ...
              {ticks.map((val, idx) => {
                ...
                return (
                  <g key={idx} transform={`translate(${x},0)`}>
                    ...
                    <text x={0} y={20} fontSize={11} fill="#6b7280" textAnchor="middle">{val}</text>
                  </g>
                );
              })}
            </g>
        ...
        {data.map((d, i) => {
          ...
          const barColor = d.color || (usePalette ? palette[i % palette.length] : null);
          ...
          return (
            ...
            >
              ...
              <rect x={labelWidth} y={6} rx={6} ry={6} width={barW} height={heightPer - 12} className="bar-fill-svg" fill={barColor || 'url(#barGradient)'} />
      ...
      {legendItems && legendItems.length > 0 && (
        <div className="chart-legend" role="list" aria-hidden="false">
          {legendItems.map((it, idx) => (
            <div key={idx} className="chart-legend-item" role="listitem">
              <div className="chart-legend-swatch" style={{ background: it.color }} aria-hidden="true" />
              <div className="chart-legend-label">{it.label}</div>
            </div>
          ))}
        </div>
      )}
